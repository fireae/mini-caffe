...
__global__ void transpose_gpu(const int nthreads, const real_t* from_data, real_t* to_data, 
...
void TransposeLayer::Forward_gpu(const vector<Blob*>& bottom, 
		const vector<Blob*>& top) {
	const int nthreads = bottom[0]->count();

	transpose_gpu  // NOLINT_NEXT_LINE(whitespace/operators)
